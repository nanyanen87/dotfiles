---
name: git-wt
description: ブランチ戦略に沿った並列開発用のworktreeスキル。ベースブランチ指定・パス規約・post-hook等、公式 --worktree にない機能を提供。公式は一時隔離向き、このスキルは本格的な並列開発向き。
---
...
Claude Code には公式の `--worktree` オプション（`EnterWorktree` ツール）が存在する。目的が異なるため、状況に応じて使い分けること。

### 公式 `--worktree` の特徴

- `.claude/worktrees/` にワークツリーを作成
- 常に HEAD から分岐（ベースブランチ指定不可）
- セッション終了時に自動クリーンアップ
- `/resume` によるセッション再開と統合
- サブエージェントの `isolation: worktree` に対応

### 比較

| 観点 | 公式 `--worktree` | このスキル (git-wt) |
|---|---|---|
| **ベースブランチ** | HEAD 固定 | main/develop/任意を選択可能 |
| **作成場所** | `.claude/worktrees/` | `../{repo}-wt/`（擬似モノレポ対応） |
| **ライフサイクル** | セッション連動・自動削除 | 手動管理（セッション跨ぎ可能） |
| **post-hook** | なし | `wt.hook` で `pnpm install` 等を自動実行 |
| **環境ファイル** | コピーなし | `wt.copyignored` で `.env` 等を自動コピー |
| **用途** | 一時的な調査・実験・サブエージェント隔離 | ブランチ戦略に沿った本格的な並列開発 |

### 判断基準

- **公式で十分**: 一時的なコード調査、使い捨ての実験、サブエージェント隔離
- **このスキルを使う**: hotfix を main から切りたい、issue ブランチを develop から作りたい、worktree を複数セッションに渡って維持したい、`pnpm install` や `.env` コピーを自動化したい
...
### 公式 `--worktree` で十分な場合（このスキルを使わない）

以下のケースでは公式機能で事足りるため、このスキルは使用しない：

- 一時的なコード調査や実験（すぐ破棄する前提）
- サブエージェントの隔離（`isolation: worktree`）
- ベースブランチやパス配置にこだわりがない単発作業
...
**重要な更新**: 公式 `--worktree` オプションとの使い分けセクションを追加
