...
```bash
# セッションIDを指定して resume
codex exec resume --full-auto <SESSION_ID> "<追加の質問>"

# 直前のセッションを resume
codex exec resume --full-auto --last "<追加の質問>"
```
...
1. `codex exec --full-auto` でプロンプトを送信（**フォアグラウンド、timeout: 300000**）
...
```bash
codex exec --full-auto "PR #123 の変更をレビューしてください。gh pr diff 123 で diff を取得して確認してください。"
```
