...
```bash
codex exec review --full-auto "$(cat ~/.codex/prompts/review.md)"
```

**注意**:
- **フォアグラウンド**（`timeout: 300000`）で実行すること。バックグラウンドだとユーザーが次のメッセージを送るまで完了通知が届かず、会話が止まる
- `codex exec --full-auto "..."` は Claude Code の Bash ツール経由だと stdin が閉じられず EOF 待ちでハングする。必ず `codex exec review` サブコマンドを使うこと
